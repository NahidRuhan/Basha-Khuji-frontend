...
interface PropertiesClientProps {
  ...
  initialLocations?: ApiResponse<Location[]>;
}

export function PropertiesClient({ initialProperties, initialCategories, initialLocations }: PropertiesClientProps) {
  ...
  const { data: locationsData } = useLocations(initialLocations);
  ...
  const [initialFilters] = useState(() => JSON.stringify(activeFilters));
  const isFiltersChanged = JSON.stringify(activeFilters) !== initialFilters;

  const { data, isLoading } = useProperties(
    activeFilters, 
    isFiltersChanged ? undefined : initialProperties
  );
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              {isLoading && !properties.length ? "Searching..." : `Found ${properties.length} results`}
            </p>
          </div>
